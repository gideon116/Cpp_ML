...
Tensor* Linear::forward_pass(const Tensor* px, const bool training, void*) 
{
    ...
    if (m_use_bias)
        out = wef::matmul(*px, W, true) + B;
    else
        out = wef::matmul(*px, W, true);
    return &out;
...
Tensor* Linear::backward_pass(const Tensor* dy, const float lr, void*) 
{
    ...
    while (dw.m_rank > W.m_rank)
        dw = wef::reducesum(dw, /*axis*/0, /*keepkims*/false);
    ...
    if (m_use_bias) 
    {
        ...
        db = *dy;
        while (db.m_rank > B.m_rank)
            db = wef::reducesum(db, /*axis*/0, /*keepkims*/false);
        db = wef::reducesum(db, 0, /*keepkims*/true); // because bias is shape = [1, bias]
        // or
...
{
    temp = *out_layer->backward_pass(dy, lr, gpu);
